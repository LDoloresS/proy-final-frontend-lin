...
const OrderDetail = () => {
    ...
    const orderC = orders.find(order => order.id == index);

    return (
        <section className="bg-gray-400 flex flex-col justify-between items-center dark:bg-gray-200">
            ...
            <div className="flex flex-col w-80 justify-between items-center border-2 border-gray-500 rounded-lg p-4 m-2">
                <p>
                    <span className="font-bold">Fecha:</span> {orderC.fecha.toLocaleDateString("en-US")}
                </p>
                <p>
                    <span className="font-bold">Total:</span> <span className="text-lg font-medium">S/ {orderC.totalPrecio}</span>
                </p>
            </div>
        </section>
    )
